...
- **A skill-forge skill** (working name `nc3-meta-skill-forge`; unbuilt). The predecessor `nc3-meta-conventions-skill` it was to be rescoped from is **retired** as of 2026-08-03 and owns nothing; the repository's `CLAUDE.md` is now the authority for naming, versioning, and file structure. A forge skill would still codify the cross-skill conventions introduced here — decoupled schema versioning, build-time schema-drift detection, the `user-config.md` per-user override layer, and the `_managed_by` registry-marker convention introduced in v0.5.0 — but nothing depends on it shipping. `CONTRIBUTING.md` points to `config/user-config.md.template` as the canonical example in the meantime.
