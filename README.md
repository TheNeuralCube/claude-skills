...
| [session-orchestrator](./skills/session-orchestrator) | 0.1.1 | [`.skill`](https://github.com/TheNeuralCube/claude-skills/releases/download/session-orchestrator-v0.1.1/session-orchestrator.skill) | A three-layer session model for multi-issue software work, where the session that decides is never the session that writes the code. Carries nine verification rules, each paid for by a real failure. **Hub-resident: see its README before installing.** |
