...
|---------|------|---------|
| 0.1.1 | 2026-08-06 | Housekeeping. `version` added to the frontmatter (it had existed only in the H1 and this table); the four required per-skill documents added; two dead cross-references removed. A portability notice now states plainly that this skill is Hub-resident and is slated to move to a private Hub-skills repository. No behavior change. |
