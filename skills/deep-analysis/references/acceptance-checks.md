...
Run checks marked "script" with `python scripts/deep_analysis_checks.py check <files>`; verify the rest by reading the named files.
...
| 10 | Filename convention | Output pattern is `{YYYY-MM-DD}_{target-slug}_{lens-tag}_deep-analysis.md` and conforms to the conventions skill | Script (`filename` command generates and validates) |
