...
- **Three migration paths in v0.6.0:**
  - Legacy migration (v0.1-era → schema "0.3" via Scenario D): existing v0.5.0 algorithm preserved verbatim. Operator confirmation token: `confirm migration`. After legacy migration, operators re-invoke for Scenario F to reach schema "0.4". Note: design spec §7.3 suggested a single-step Scenario D path producing schema 0.4 directly with operator-declared role; v0.6.0 preserves the v0.5.0 algorithm verbatim per Phase 2 scope and treats the legacy → Scenario F sequence as a two-step path. Single-step legacy migration to schema 0.4 deferred to a future release if operator demand surfaces.
  - Upgrade migration (schema "0.2" → schema "0.3" via Scenario E): existing v0.5.0 algorithm preserved verbatim. Operator confirmation token: `confirm upgrade`. After Scenario E, operators re-invoke for Scenario F to reach schema "0.4".
