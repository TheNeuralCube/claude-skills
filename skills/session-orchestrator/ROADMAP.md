...
| **A tenth rule for post-deployment monitoring** | Candidate. Rules 1 and 2 cover the gap between merged and deployed, but nothing yet covers what to watch after a deploy lands. Would need a real failure to anchor it, per the skill's own authoring standard. |
...
- **Single-operator provenance.** Nine rules from one operator's four-day run. They are real and each was paid for, but they are not yet known to generalize.
