...
# Defaults (project-context v0.5.0)
...
If a value appears here, it can be overridden in user-config or org-config. If a value does not appear here, it is not user-tunable in v0.5.0.
...
These budgets apply to `project-context.md`. `entities.md` and `project-context-archive.md` have no enforced ceilings in v0.5.0.
...
The surface guard cannot be disabled in v0.5.0 (the skill is fundamentally inappropriate for Claude Code surfaces). Migration is opt-in only in the sense that the operator confirms the migration brief; detection is always on.
...
ID prefixes are not user-configurable in v0.5.0. They are part of the schema contract (`schema_version: "0.3"`).
...
`read_order` is not user-configurable in v0.5.0. It is part of the schema contract.

## 11. What is NOT configurable in v0.5.0
...
- The schema (must conform to `schema_version: "0.3"`).
